...
void setup() {
  ...
  if (EEPROM.read(1022) != 120 || resetSettings) {
    EEPROM.write(1022, 120);
    for (byte i = 0; i < 10; i++) {
    ...
    for (byte i = 0; i < PID_CH_AMOUNT; i++) {
      EEPROM.put(i * 20 + 500, PID[i]);
      if (i < 4) EEPROM.put(i * 6 + 700, dawn[i]);
    }
  }
  ...
  for (byte i = 0; i < PID_CH_AMOUNT; i++) {
    EEPROM.get(i * 20 + 500, PID[i]);
    if (i < 4) EEPROM.get(i * 6 + 700, dawn[i]);
  }
