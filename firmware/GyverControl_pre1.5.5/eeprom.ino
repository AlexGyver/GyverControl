void updateEEPROM(byte channel) {
  if (EEPROM_updFlag) {
    ...
    if (channels[channel].mode == 4 || channels[channel].mode == 5) {
      ...
      if (channels[channel].mode == 4) EEPROM.put(curPWMchannel * 20 + 500, PID[curPWMchannel]);
      if (channels[channel].mode == 5 && curPWMchannel != 6) EEPROM.put(curPWMchannel * 6 + 700, dawn[curPWMchannel]);
    }
