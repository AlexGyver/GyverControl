void drawPlot(float *plot_array) {
#if (USE_PLOTS == 1)
  ...
  lcd.setCursor(0, 2); lcd.print('>'); lcd.print(plot_array[14], 1);
  ...
  for (byte i = 0; i < 15; i++) {                  // каждый столбец параметров
    ...
    if (plot_array[i] > min_value)
      infill = floor((float)(plot_array[i] - min_value) / (max_value - min_value) * 4 * 10);
    else infill = 0;
...
void redrawDebug() {
  ...
  // 1 влажн
  clearLine2(1);
  lcd.setCursor(0, 1); lcd.print((int)(sensorVals[1])); lcd.write(37);
  ...
  lcd.setCursor(0, 2); lcd.print(sensorVals[0], 1); if (sensorVals[0] >= 0 && sensorVals[0] < 100) lcd.write(223);
  ...
  for (byte i = 0; i < 4; i++) {  // для всех строк
    ...
    if (index >= 0) {
      ...
      switch (index) {
        ...
        case 10: lcd.print(settings.comSensPeriod); break;
        case 11: switch (settings.plotMode) {
