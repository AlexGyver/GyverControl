void timersTick() {   // каждую секунду
        ...
        case 3:   // ---------------------- если датчик ----------------------
          if (millis() - timerMillis[curChannel] >= activeChannel.sensPeriod * 1000L) {
            ...
            */
            ...
            if (checkHysteresis(curChannel) == 1) channelStates[curChannel] = activeChannel.direction;
            else if (checkHysteresis(curChannel) == 2) channelStates[curChannel] = !activeChannel.direction;
          }
          break;
        case 4:   // ---------------------- если ПИД ----------------------
          break;
  ...
  for (byte relay = 0; relay < 10; relay++) {
    ...
    if (!thisCh.type && thisCh.state && thisCh.mode != 4 && thisCh.mode != 5) {
  ...
  for (byte relay = 0; relay < 10; relay++) {
    channelsStruct thisCh = loadChannel(relay);
    if (!thisCh.type && thisCh.state && thisCh.relayType == 2
        && thisCh.mode != 4 && thisCh.mode != 5) {    // только для ОБЩИХ каналов реле, кроме ПИД и рассвета
...
#if (SMOOTH_SERVO == 1)
  if (activeChannel.state) servo1.setTargetDeg(pwmVal[4]);
#else
  if (activeChannel.state) servo1.write(pwmVal[4]);
#endif
...
#if (SMOOTH_SERVO == 1)
  if (activeChannel.state) servo2.setTargetDeg(pwmVal[5]);
#else
  if (activeChannel.state) servo2.write(pwmVal[5]);
#endif
...
void checkDawn(byte curChannel) {
  ...
  if (curChannel == 2 || curChannel == 3 || curChannel == 7 || curChannel == 8) {
    ...
    if (nowTime < (timeStart)) {   // до рассвета (ночь)
      if (activeChannel.direction) thisSignal = activeDawn.minV;
      else thisSignal = activeDawn.maxV;
    } else if (nowTime < (timeStart + activeDawn.dur1 * 60)) { // рассвет
      if (activeChannel.direction)
        thisSignal = map((nowTime - timeStart), 0, activeDawn.dur1 * 60, activeDawn.minV, activeDawn.maxV);
      else
        thisSignal = map((nowTime - timeStart), 0, activeDawn.dur1 * 60, activeDawn.maxV, activeDawn.minV);
    } else if (nowTime < (timeStop)) {  // день
      if (activeChannel.direction) thisSignal = activeDawn.maxV;
      else thisSignal = activeDawn.minV;
    } else if (nowTime < (timeStop + activeDawn.dur2 * 60)) { // закат
      if (activeChannel.direction)
        thisSignal = map((nowTime - timeStop), 0, activeDawn.dur2 * 60, activeDawn.maxV, activeDawn.minV);
      else
        thisSignal = map((nowTime - timeStop), 0, activeDawn.dur2 * 60, activeDawn.minV, activeDawn.maxV);
    } else {  // ночь
      if (activeChannel.direction) thisSignal = activeDawn.minV;
      else thisSignal = activeDawn.maxV;
    }
