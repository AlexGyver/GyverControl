...
/*
  ...
  - Добавлен редим ПИД для каналов реле 2 и 3, обоих каналов серво, канала привода
  - Добавлен режим РАССВЕТ для каналов реле 2 и 3 и обоих каналов серво
  - Каналы ПИД и рассвета помечены *
  - На экране отладки каналы ПИД и рассвета выводятся графиком
  - Добавлена настройка, позволяющая убрать из кода датчик BME280
  - К режиму Day добавлена настройка Sensor, смотри документацию
  - Значения датчиков сведены к 8 битам (0 - минимум, 255 - максимум)
  - В режиме датчиков можно установить максимум меньше минимума, это инвертирует работу режима
*/
...
// цифровой датчик температуры и влажности DHT11/DHT22
...
struct channelsStruct {
  ...
  boolean sensorDay = false;  // режим сутки + сенсор
...
void setup() {
  ...
  if (EEPROM.read(1022) != 10 || resetSettings) {
    EEPROM.write(1022, 10);
    for (byte i = 0; i < 10; i++) {
