...
void scheduleSett(int dir) {
  switch (arrowPos) {
    case 0:
      changeChannel(dir);
      break;
    case 1: incr(&setSchedule.pidChannel, dir, 7); break;
  ...
  if (arrowPos > 4) {
    incr(&setSchedule.setpoints[arrowPos - 5], dir, 500);
  }
}
#endif
...
#if (SCHEDULE_NUM > 0)
        incr(&arrowPos, 1, (4 + loadSchedule(currentChannel - 10).pointAmount));
#endif
      }
      drawArrow();
