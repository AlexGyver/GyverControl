...
void controlTick() {
  ...
  if (enc.isTurn()) {
        ...
        if (currentChannel >= 0) {
          ...
          if (navDepth == 0) {  // настройки канала
            ...
            switch (channels[currentChannel].mode) {
              case 0: if (++arrowPos > 7) arrowPos = 7; break;
              case 1: if (++arrowPos > 4) arrowPos = 4; break;
              case 2: if (++arrowPos > 15) arrowPos = 15; break;
              case 3: if (++arrowPos > 5) arrowPos = 5; break;
              case 4: if (++arrowPos > 9) arrowPos = 9; break;
              case 5: if (++arrowPos > 7) arrowPos = 7; break;
            }
          }
    ...
    if (currentChannel == -2) redrawMainSettings();   // костыль для настроек
    if (currentChannel >= 0 && navDepth == 1 && thisMode >= 4) redrawScreen();  // костыль для пида и рассвета
  }
}
