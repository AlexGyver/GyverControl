void drawPlot(byte pos, byte row, byte width, byte height, float *plot_array) {
#if (USE_PLOTS == 1)
  ...
  lcd.setCursor(0, 1); lcd.print(max_value, 1); //lcd.print(F("  "));
  lcd.setCursor(0, 2); lcd.print('>'); lcd.print(plot_array[14], 1); //lcd.print(F("  "));
  lcd.setCursor(0, 3); lcd.print(min_value, 1); //lcd.print(F("  "));
...
void redrawPlot() {
  ...
  switch (debugPage) {
    case 1: lcd.print(F("TEMP"));
      drawPlot(5, 3, 15, 4, tempDay);
      break;
    case 2: lcd.print(F("HUM"));
      drawPlot(5, 3, 15, 4, humDay);
      break;
    case 3: lcd.print(F("SEN1"));
#if (THERM1 == 1)
      drawPlot(5, 3, 15, 4, sensDay_0);
#else
      drawPlot(5, 3, 15, 4, sensDay_0);
#endif
      break;
...
#if (THERM2 == 1)
      drawPlot(5, 3, 15, 4, (float*)sensDay_1);
#else
      drawPlot(5, 3, 15, 4, (float*)sensDay_1);
#endif
      break;
...
#if (THERM3 == 1)
      drawPlot(5, 3, 15, 4, (float*)sensDay_2);
#else
      drawPlot(5, 3, 15, 4, (float*)sensDay_2);
#endif
      break;
...
#if (THERM4 == 1)
      drawPlot(5, 3, 15, 4, (float*)sensDay_3);
#else
      drawPlot(5, 3, 15, 4, (float*)sensDay_3);
#endif
      break;
  ...
  if (currentLine == 2 || currentLine == 4) {
    ...
    switch (curSetMode) {
      ...
      case 4:
        ...
        lcd.setCursor(11, 2);
        lcd.print("set:"); lcd.print(PID[curPWMchannel].setpoint, 1);
#endif
        break;
      case 5:
#if (USE_DAWN == 1)
...
void redrawDebug() {
  ...
  lcd.setCursor(0, 1); lcd.print("h"); lcd.print((int)(sensorVals[1]));
  lcd.setCursor(4, 1);
#if (DALLAS_SENS1 == 1)
  ...
  else lcd.print((int)sensorVals[3]);
  ...
  if (THERM4) {
    ...
    if ((int)sensorVals[4] < 10) lcd.print(F("  "));
    else if ((int)sensorVals[4] < 100) lcd.print(F(" "));
  }

  lcd.setCursor(0, 2); lcd.print(F("t")); lcd.print(sensorVals[0], 1); if (sensorVals[0] >= 0 && sensorVals[0] < 100) lcd.write(223);
...
void redrawMainSettings() {
  /*
    ...
    lcd.setCursor(11, 0); lcd.print(settingsPageNames[0]);
    if (settings.backlight) lcd.print(F("On"));
    else lcd.print(F("Off"));
    }
    if (currentLine == 1 || currentLine == 4) {
    clearLine2(1);
    lcd.setCursor(0, 1); lcd.print(settingsPageNames[1]); lcd.print(settings.backlTime);
    lcd.setCursor(13, 1); lcd.print(settingsPageNames[2]); lcd.print(settings.drvSpeed);
    }
    if (currentLine == 2 || currentLine == 4) {
    clearLine2(2);
    lcd.setCursor(0, 2); lcd.print(settingsPageNames[3]); lcd.print(settings.srv1_Speed);
    lcd.setCursor(10, 2); lcd.print(settingsPageNames[4]); lcd.print(settings.srv1_Acc, 1);
    }
    if (currentLine == 3 || currentLine == 4) {
    clearLine2(3);
    lcd.setCursor(0, 3); lcd.print(settingsPageNames[5]); lcd.print(settings.srv2_Speed);
    lcd.setCursor(10, 3); lcd.print(settingsPageNames[6]); lcd.print(settings.srv2_Acc, 1);
    }*/
  lcd.clear();
  ...
  for (byte i = 0; i < 4; i++) {  // для всех строк
    ...
    if (arrowPos == 4 * screenPos + i) smartArrow();  // рисуем стрелку
    ...
    if (index >= 0) {
      ...
      spaceColon();
      switch (index) {
        case 0: if (settings.backlight) lcd.print(F("On"));
          else lcd.print(F("Off")); break;
        case 1: lcd.print(settings.backlTime); break;
        case 2: lcd.print(settings.drvSpeed); break;
        case 3: lcd.print(settings.srv1_Speed); break;
        case 4: lcd.print(settings.srv1_Acc, 1); break;
        case 5: lcd.print(settings.srv2_Speed); break;
        case 6: lcd.print(settings.srv2_Acc, 1); break;
        case 7: lcd.print(rtc.getDate()); break;
        case 8: lcd.print(rtc.getMonth()); break;
        case 9: lcd.print(rtc.getYear()); break;
      }
    }
  }
}
