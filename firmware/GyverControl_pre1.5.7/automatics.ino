void timersTick() {   // каждую секунду
  ...
  for (byte relay = 0; relay < 7; relay++) {
    if (channels[relay].state && channels[relay].mode != 4 && channels[relay].mode != 5) {
  ...
  for (byte relay = 0; relay < 7; relay++) {
    if (channels[relay].state && channels[relay].relayType == 2
  ...
  if (SERVO1_RELAY) {   // если реле
    ...
    if (channels[7].mode != 4 && channels[7].mode != 5) {
      if (channelStates[7]) servoPos[0] = settings.minAngle[0];
  ...
  if (SERVO2_RELAY) {   // если реле
    ...
    if (channels[8].mode != 4 && channels[8].mode != 5) {
      if (channelStates[8]) servoPos[1] = settings.minAngle[1];
...
void checkDawn(byte curChannel) {
  ...
  /* // менее плавный рассвет (по минутам)
  ...
  switch (curChannel) {
    case 2: pwmVal[2] = thisSignal;
      break;
    case 3: pwmVal[3] = thisSignal;
      break;
